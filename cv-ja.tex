...
\begin{description}
 \item[2024年11月-現在] 株式会社Jij ソフトウェア開発部（リードエンジニア）．
  RustによるPython向け数理最適化モデリング言語JijModelingの設計・開発に従事．
  特に，次期バージョンJijModeling 2の刷新の完全な再設計・実装を主導．成果について国内外の研究集会で登壇・発信を行う．
...
\phantomsection\addcontentsline{toc}{subsection}{Jijにおける主な業績}
\subsection*{Jijにおける主な業績}

\begin{itemize}
  \item 数理最適化モデリング言語JijModelingの次期バージョン2のゼロからの設計・実装を主導
    \begin{itemize}
      \item 表層言語（Python）との疎結合化の達成
      \item 記号処理による検出機構の強化
      \item 言語機能の設計・実装
      \item 専用の型システムの設計・実装
    \end{itemize}
  \item 上記に関連する学術的・技術的成果の国内外での講演
\end{itemize}

\phantomsection\addcontentsline{toc}{subsection}{DeepFlowにおける主な業績}
...
\subsection*{査読付き会議論文}
\noindent
June 2025, \emph{Optimizing Optimizations: Case Study on Detecting Specific Types of Mathematical Optimization Constraints with E-Graphs in JijModeling}. EGARPHS '25 (colocated with PLDI '25), 2025, Seoul, South Korea. (Joint work with T. Shimizu and T. Teramura)

\noindent
May 2025, \emph{数理最適化モデル記述ツール JijModeling のご紹介}. 2025年度 人工知能学会全国大会 (第39回) 数理最適化OS, Osaka, Japan. (Joint work with T. Shimizu and T. Teramura)
...
\phantomsection\addcontentsline{toc}{subsection}{商業技術記事}
\subsection*{商業技術記事}
\noindent
December 2025, \emph{do式でわかるモナドとその仲間たち}. 『n月刊ラムダノート』Vol.5 No.2（2025）所収. ラムダノート株式会社.

\phantomsection\addcontentsline{toc}{subsection}{技術系会議}
\subsection*{技術系会議}
\noindent
June 2025, \emph{Rust世界の二つのモナド──Rust でも do 式をしてプログラムを直感的に記述する件について}. 関数型まつり, Tokyo, Japan.

\begin{refsection}
  \phantomsection\addcontentsline{toc}{subsection}{論文}
  \nocite{Ishii:2025b,JijModeling-jsai:2025,ommx:2025,Ishii:2021tp,ISHII:2016sf,ISHII:2018ek,ISHII:2019aa,Ishii:2021tp,Kiselyov:2015xy}
...
\begin{description}
  ...
  \item[Rust] 上級．Rustによる言語処理系のゼロからの設計・実装経験あり．
...
\begin{description}
  ...
  \item[EDSLの設計] ドメインに応じたHaskell内，Rust内，Python内DSLの設計・実装経験豊富．
  \item[線型型・所有権システム] リソース割り当てや管理などに利用．
  \item[コンテナ技術] Docker, Singularity
  \item[CI/CD] GitHub Actions, GitLab CI/CD
  \item[形式手法] 性質ベーステスト（QuickCheck, Falsify, hypothesis），リグレッションテスト，依存型によるコンパイル時検証，型システムの設計・実装
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Rustにもプロフェッショナルな業務経験があり，両者の長所を活かした開発を行いたい．
  \end{itemize}
